...
- Gộp placement thành **hàng (row)** theo Y (tolerance = ~2× chiều cao tag).
- Trong mỗi hàng có ≥ 2 tag:
  - Tính **Y trung bình** của cả hàng.
  - Kiểm tra: nếu đổi tất cả tag trong hàng sang Y trung bình mà **không gây collision** với tag khác hàng → mới áp dụng.
  - Gán lại **TagLocation.Y** và **EstimatedTagBounds** cho từng placement trong hàng.

Kết quả: tag cùng hàng có cùng Y, bản vẽ gọn và dễ đọc.
