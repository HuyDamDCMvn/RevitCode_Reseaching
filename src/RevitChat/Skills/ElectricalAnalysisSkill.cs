    ...
    public class ElectricalAnalysisSkill : BaseRevitSkill
    {
        ...
        private string GetVoltageDrop(Document doc, Dictionary<string, object> args)
        {
            ...
            var results = circuits.Select(c =>
            {
                ...
                double dropPercent = voltage > 0 ? (load * length * 0.00001 / voltage) * 100 : 0;

                return new
                {
                    circuit_id = c.Id.Value,
                    circuit_number = c.CircuitNumber,
                    length_ft = Math.Round(length, 1),
            ...
            return JsonSerializer.Serialize(new { max_drop_percent = maxDrop, circuits_checked = results.Count, over_limit = overLimit, circuits = results }, JsonOpts);
        }
        ...
        private string GetPhaseBalance(Document doc, Dictionary<string, object> args)
        {
            ...
            foreach (var c in circuits)
            {
                ...
                if (poles == 1) phaseA += load;
                else if (poles == 2) { phaseA += load / 2; phaseB += load / 2; }
                else { phaseA += load / 3; phaseB += load / 3; phaseC += load / 3; }
            }

            double total = phaseA + phaseB + phaseC;
            double avg = total / 3;
            double maxImbalance = total > 0 ? Math.Max(Math.Max(Math.Abs(phaseA - avg), Math.Abs(phaseB - avg)), Math.Abs(phaseC - avg)) / avg * 100 : 0;
